// Riya AI Companion - External API Integration (rudra-here.onrender.com)
...
// Function to generate voice reply (Optional: Requires VoiceRSS API Key)
...
module.exports.config = {
    name: "Riya", // <<<--- नाम Riya ही रहेगा
    version: "2.1.0", 
    hasPermssion: 0, 
    credits: "Rudra + API from Angel code + Logging & User Name by Gemini + Code Generation Ability",
    description: "Riya, your AI companion who is smart, can generate code, has UID specific behavior, and nuanced reactions. Responds only when triggered. Modified for 3-4 line replies (with code exceptions).", // <<<--- विवरण अपडेट किया गया
    commandCategory: "AI-Companion",
    usages: "Riya [आपका मैसेज] / Reply to Riya", // <<<--- उपयोग अपडेट किया गया
    cooldowns: 2,
};

const chatHistories = {};
const AI_API_URL = "https://rudra-here.onrender.com/chat"; // <<<--- AI_API_URL को अपडेट किया गया है
...
module.exports.handleEvent = async function ({ api, event }) {
    try {
        ...
        const isRiyaTrigger = body?.toLowerCase().startsWith("riya"); // <<<--- ट्रिगर Riya ही रहेगा
        const isReplyToRiya = messageReply?.senderID === api.getCurrentUserID(); // <<<--- रिप्लाई Riya ही रहेगा
        if (!(isRiyaTrigger || isReplyToRiya)) {
            return; // Ignore messages that are not triggers
        }

        console.log("--- Riya HandleEvent ---"); // <<<--- लॉग Riya ही रहेगा
        console.log("Riya's Bot ID:", api.getCurrentUserID()); // <<<--- लॉग Riya ही रहेगा
        ...
        let userMessage;
        if (isRiyaTrigger) {
            userMessage = body.slice(4).trim(); // <<<--- slice(4) ही रहेगा (क्योंकि 'Riya' में 4 अक्षर हैं)
        } else { // isReplyToRiya
        ...
        try {
            ...
            if (!botReply || botReply.toLowerCase().startsWith("user:") || botReply.toLowerCase().startsWith("riya:")) { // <<<--- Riya ही रहेगा
                ...
                chatHistories[senderID].push(`Riya: ${botReply}`); // <<<--- Riya ही रहेगा
            }
            ...
            if (senderID === ownerUID) {
                // Footers for Owner
                if (isBoldMode || hornyMode) {
                     replyText = `${botReply} 😉🔥💋\n\n_Your charmingly naughty Riya... 😉_`; // <<<--- Riya ही रहेगा
                } else {
                     replyText = `${botReply} 😊💖✨`;
                }
            } else {
            ...
            if (isReplyToRiya && messageReply) { // <<<--- Riya ही रहेगा
            ...
            console.error("Riya AI API Error:", apiError); // <<<--- लॉग Riya ही रहेगा
            api.sendTypingIndicator(threadID, false);
        ...
        console.error("Riya Bot Catch-all Error:", err); // <<<--- लॉग Riya ही रहेगा
